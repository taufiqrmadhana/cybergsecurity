...
const MainContent = () => {
  ...
  const handleSummarize = async (query: string) => {
    ...
    try {
      ...
      setLlmResponse('Ringkasan gagal diambil.')
    }
    ...
    <div className="flex-1 flex flex-col">
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex bg-slate-100 rounded-lg p-1">
            <button
              ...
              className={`px-4 py-2 rounded-md text-sm font-medium transition-colors ${currentView === 'viewer' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-600 hover:text-slate-900'}`}
            >
              Document Viewer
            </button>
            <button
              onClick={() => setCurrentView('editor')}
              className={`px-4 py-2 rounded-md text-sm font-medium transition-colors ${currentView === 'editor' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-600 hover:text-slate-900'}`}
            >
              Word Editor
            </button>
          </div>
      ...
      <div className="flex-1 flex overflow-hidden">
        {currentView === 'viewer' ? (
          <>
            <DocumentListSidebar />
            {currentFile ? <EditorPanel /> : <ViewerLandingScreen />}
            <AIAssistantPanel
